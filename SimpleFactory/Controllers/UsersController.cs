...
namespace SimpleFactory.Controllers
{
    public class UsersController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "UserId,Username,Password,ConfirmPassword,IsLogged")] User user)
        {
            if (ModelState.IsValid)
            {
                var listOfUsers = db.Users.Select(u => u.Username).ToList();

                if (!listOfUsers.Contains(user.Username))
                {
                    db.Users.Add(user);
                    db.SaveChanges();
                }
